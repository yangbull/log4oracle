CREATE OR REPLACE PACKAGE LogManager AS
    ...
    LOOP
      ...
      ELSE
        ...
        IF ( cnt = (2+DEPTH) ) THEN
          ...
          prgname := LTRIM(RTRIM(SUBSTR( LINE, N+1 )));
          EXIT;
        END IF;
      END IF;
    END LOOP;
...
--           BEGIN NULL; END;
  ...
  FUNCTION isEnabled(lvl VARCHAR2/*, mkr VARCHAR2*/) RETURN BOOLEAN IS
  BEGIN
    RETURN (CASE lvl
  ...
  BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE(TO_CHAR(SYSTIMESTAMP,'YYYY-MM-DD"T"HH:MI:SSXFF6')||' '||RPAD(lvl,5)||' '
                       ||REPLACE(prgname,' ')||'('||lineno||')'||RTRIM(' '||marker)||' - '||msg);
  END;
